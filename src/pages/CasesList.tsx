...
import { generateFullCaseReport } from "@/lib/caseReportGenerator";
...
const CasesList = () => {
  ...
  const handlePrint = useCallback(async (c: Case) => {
    setIsPrinting(true);
    try {
      await generateFullCaseReport(c, userIP);
    } catch (err) {
  ...
  return (
    <PlatformLayout>
      ...
      {/* Report generation is now handled by caseReportGenerator */}
