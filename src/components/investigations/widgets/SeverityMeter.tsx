...
export const SeverityMeter = () => {
  ...
  const severityConfig = [
    { level: "critical", label: t('widgets.critical'), count: severityCounts.critical, icon: AlertTriangle, color: "bg-red-500 text-white", barColor: "bg-red-500" },
    { level: "high", label: t('widgets.high'), count: severityCounts.high, icon: AlertCircle, color: "bg-orange-500 text-white", barColor: "bg-orange-500" },
    { level: "medium", label: t('widgets.medium'), count: severityCounts.medium, icon: Shield, color: "bg-yellow-500 text-white", barColor: "bg-yellow-500" },
    { level: "low", label: t('widgets.low'), count: severityCounts.low, icon: Info, color: "bg-blue-500 text-white", barColor: "bg-blue-500" },
  ];
      ...
      <CardContent>
        <div className="flex h-3 rounded-full overflow-hidden mb-4">
          {severityConfig.map((sev) => {
            ...
            return (
              <div key={sev.level} className={`${sev.barColor} transition-all`} style={{ width: `${width}%` }} />
            );
          })}
        </div>
        <div className="grid grid-cols-2 gap-2">
          {severityConfig.map((sev) => {
            const Icon = sev.icon;
            return (
              <div key={sev.level} className="flex items-center justify-between p-2 rounded-lg bg-accent/30">
