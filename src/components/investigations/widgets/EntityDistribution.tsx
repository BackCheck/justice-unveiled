...
export const EntityDistribution = () => {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case "antagonist": return "hsl(0, 84%, 60%)";
      case "protagonist": return "hsl(217, 91%, 60%)";
      case "official": return "hsl(262, 83%, 58%)";
      case "neutral": return "hsl(215, 14%, 34%)";
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            {t('widgets.entityDistribution')}
          </CardTitle>
          <Badge variant="secondary" className="text-xs">{entities.length} {t('widgets.total')}</Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="pt-2 border-t border-border/50">
              <p className="text-[10px] text-muted-foreground mb-1">{t('widgets.topTypes')}</p>
