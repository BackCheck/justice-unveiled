...
AS $$
  SELECT jsonb_build_object(
    ...
    'total_connections', (
      ...
      WHERE (p_case_id IS NULL OR case_id = p_case_id OR case_id IS NULL)
    ),
    ...
    'ai_extracted_events', (SELECT COUNT(*) FROM extracted_events WHERE extraction_method = 'ai_analysis' AND (p_case_id IS NULL OR case_id = p_case_id)),
    'ai_extracted_entities', (SELECT COUNT(*) FROM extracted_entities WHERE source_upload_id IS NOT NULL AND (p_case_id IS NULL OR case_id = p_case_id)),
    'inferred_connections', (
      SELECT COUNT(*) FROM entity_relationships 
      WHERE is_verified = false AND (p_case_id IS NULL OR case_id = p_case_id OR case_id IS NULL)
    ),
    'documents_analyzed', (SELECT COUNT(DISTINCT eu.id) FROM evidence_uploads eu INNER JOIN document_analysis_jobs daj ON daj.upload_id = eu.id WHERE daj.status = 'completed' AND (p_case_id IS NULL OR eu.case_id = p_case_id)),
    'events_by_category', (
    ...
    'connections_this_week', (SELECT COUNT(*) FROM entity_relationships WHERE created_at >= NOW() - INTERVAL '7 days' AND (p_case_id IS NULL OR case_id = p_case_id OR case_id IS NULL)),
    'connections_last_week', (SELECT COUNT(*) FROM entity_relationships WHERE created_at >= NOW() - INTERVAL '14 days' AND created_at < NOW() - INTERVAL '7 days' AND (p_case_id IS NULL OR case_id = p_case_id OR case_id IS NULL))
  );
$$;